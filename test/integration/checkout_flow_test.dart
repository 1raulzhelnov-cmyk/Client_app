...
void main() {
  ...
  test('placeOrder с оплатой наличными добавляет инструкцию и комиссию', () async {
    final apiService = MockApiService();
    final firebaseAuth = MockFirebaseAuth();
    final firebaseUser = MockUser();
    final cartUpdateSpy = _SpyCartUpdateNotifier();
    ...
    when(
      ...
      (_) async => right(<String, dynamic>{
        'order': <String, dynamic>{
          ...
          'address': const <String, dynamic>{
            'id': 'addr-2',
            'formatted': 'Москва, пр-т Тестовый, 7',
            'lat': 55.75,
            'lng': 37.62,
          },
          'status': 'placed',
    ...
    notifier.updateCashInstructions('Оставьте у двери');
    ...
    expect(state.order.cashInstructions, equals('Оставьте у двери'));
    ...
    expect(captured['paymentMethod'], equals('cash'));
    expect(captured.containsKey('paymentMethodId'), isFalse);
    expect(captured.containsKey('paymentIntentId'), isFalse);
    expect(captured['cashInstructions'], equals('Оставьте у двери'));
    expect(captured['cashFee'], closeTo(expectedFee, 0.0001));
  });
}
